...
def gerar_pdf_fiel(d):
    ...
    pdf.campo("NOME", d['nome'], 120)
    pdf.campo("CPF/CNPJ", d['cpf'], 70, ln=1)
    ...
    pdf.campo("NOME", d['cnome'], 120)
    pdf.campo("CPF", d['ccpf'], 70, ln=1)
    pdf.campo("NACIONALIDADE", d['cnac'], 65)
    pdf.campo("PROFISSÃO", d['cprof'], 65)
    pdf.campo("CELULAR", d['cfone'], 60, ln=1)
    pdf.ln(2)
    ...
    # 5. TEXTO LEGAL E REGRAS (Fiel ao seu PDF)
    pdf.ln(4)
...
if aba == "Admin":
    ...
    if st.text_input("Senha Admin", type="password") == "admin123":
        arq = st.file_uploader("Upload da Planilha Frei Galvão", type=['xlsx'])
        if arq:
    ...
    st.header("📝 Formulário Oficial - Home Buy")
    if "Frei Galvão" in st.session_state['loteamentos']:
        ...
        with st.form("form_homebuy"):
            st.subheader("1. Seleção do Lote")
            ...
            if st.form_submit_button("Gerar Proposta Idêntica"):
                ...
                info = {
                    ...
                    'cnome': cnome, 'ccpf': ccpf, 'cnac': cnac, 'cprof': cprof,
        ...
        if st.session_state.pdf_data:
            st.download_button("📥 BAIXAR PDF (FORMATO HOME BUY)", st.session_state.pdf_data, f"Proposta_{st.session_state.u_ref}.pdf")
    else:
